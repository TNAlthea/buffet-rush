...
local async = require "gamelogic.emthree.internal.async"
local utils = require "gamelogic.emthree.internal.utils"
...
--
-- Count and remove blocks that are part of matches, then call callback
--
...
local function swap_slots(board, slot1, slot2)
	...
	if not board.on_swap(board, slot1, slot2) then
		...
		if #hn1 < 2 and #hn2 < 2 and #vn1 < 2 and #vn2 < 2 then
			...
			msg.post("gui#SPGUI", "reduce_turns")
		end
	end
	M.stabilize(board)
end
...
local combos = 0
-- counting how many combo created per turn
function comboCount()
	combos = combos + 1
end

function passCombo ()
	msg.post("gui#SPGUI", "combos", {combos = combos})
end
